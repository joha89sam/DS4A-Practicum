...
def make_line_plot(state_dropdown, start_date, end_date):
    ...
    dfSE_1['Año/mes terminación laboralInt'] = [int(i[1:]) for i in dfSE_1['Año/mes terminación laboral']]

    #dfSE_zona = dfSE_1
    dfSE_zona = dfSE_1[dfSE_1["Año/mes terminación laboralInt"] >= 201802]
    dfSE_zona = dfSE_zona.groupby(['Año/mes terminación laboral', 'Zona']).count().reset_index().sort_values(
        by=["Año/mes terminación laboral"], ascending=True, ignore_index=True)
    Line_fig = px.bar(dfSE_zona, x="Año/mes terminación laboral", y="numeroDocumento", color="Zona")
    Line_fig.update_layout(title='SE Zona', paper_bgcolor="#F8F9F9").update_xaxes(categoryorder="category ascending")
    ...
    dfPAC_Zona = dfPAC_1.groupby(
        ['Año/mes radicación solicitud beneficiario', 'ARG_CDESCRIPCION']).count().reset_index()
    ...
    Scatter_fig = px.bar(dfPAC_Zona, x="Año/mes radicación solicitud beneficiario", y="MPC_NID",
                         color="ARG_CDESCRIPCION")
    Scatter_fig.update_layout(title='PaC Zona', paper_bgcolor="#F8F9F9")
