...
dfSE = dfSE.drop_duplicates(['numeroDocumento'])
#dfPAC = dfPAC.drop_duplicates(['MPC_NID'])
varFilterMunicipio = ''
...
def make_line_plot(state_dropdown, start_date, end_date):
    ...
    dfSE_edad = dfSE_1
    dfSE_edad['Año/mes terminación laboral'] = [("_" + i[0:4] + i[5:7]) for i in dfSE_1['Fecha de terminación laboral']]

    dfSE_edad['Año/mes terminación laboralInt'] = [int(i[1:]) for i in dfSE_edad['Año/mes terminación laboral']]
    dfSE_edad = dfSE_edad[dfSE_edad["Año/mes terminación laboralInt"] >= 201811]
    SE_edad = dfSE_edad.groupby(['Año/mes terminación laboral', 'estadoCivil']).count().reset_index()
    Line_fig = px.bar(SE_edad, x="Año/mes terminación laboral", y="numeroDocumento", color="estadoCivil")
    Line_fig.update_layout(title='SE edad', paper_bgcolor="#F8F9F9").update_xaxes(categoryorder="category ascending")




    dfPAC_edad = dfPAC_1
    dfPAC_edad['Año/mes radicación solicitud beneficiario'] = (dfPAC_edad['FEC_RADICA_SOLICITUD_BENEFICIARIO'] / 100)
    dfPAC_edad['Año/mes radicación solicitud beneficiario'] = [int(i) for i in
                                                               dfPAC_1['Año/mes radicación solicitud beneficiario']]

    dfPAC_edad['fecha_nacimiento'] = (dfPAC_edad['FEC_NACIMIENTO_BENEFICIARIO'] / 100)
    dfPAC_edad['fecha_nacimiento'] = [int(i) for i in dfPAC_1['fecha_nacimiento']]
    dfPAC_edad['edad'] = [2020 - int(i / 100) + (1 if int(i / 100) >= 7 else 0) for i in dfPAC_edad['fecha_nacimiento']]
    dfPAC_edad = dfPAC_edad[dfPAC_edad['Año/mes radicación solicitud beneficiario'] > 201612]
    dfPAC_edad['Año/mes radicación solicitud beneficiario'] = ["_" + str(i) for i in
                                                               dfPAC_edad['Año/mes radicación solicitud beneficiario']]
    Scatter_fig = px.box(dfPAC_edad, x="Año/mes radicación solicitud beneficiario", y="edad")  # , points="all")
    Scatter_fig.update_layout(title='PaC edad', paper_bgcolor="#F8F9F9").update_xaxes(categoryorder = "category ascending")
    ...
    dfSE_edad['Año/mes terminación laboralInt'] = [int(i[1:]) for i in dfSE_edad['Año/mes terminación laboral']]
    dfSE_edad = dfSE_edad[dfSE_edad["Año/mes terminación laboralInt"] >= 201408]
    dfSE_edad = dfSE_edad.groupby(['Año/mes terminación laboral', 'Días Cotizados PILA']).count().reset_index()
    Treemap_fig = px.box(dfSE_edad, x="Año/mes terminación laboral", y="Días Cotizados PILA")  # , points="all")
    Treemap_fig.update_layout(title='SE edad', paper_bgcolor="#F8F9F9").update_xaxes(
